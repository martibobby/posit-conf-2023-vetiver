...
v <- vetiver_model(inspection_fit, 
                   model_name = "inspections-result-rstats")
board <- board_connect()
...
```{r}
vetiver_deploy_rsconnect(board, "vetiver2023/inspections-result-rstats")
```
...
```{r}
url <- "https://spectral-bullfinch.d01bf.fleeting.rstd.io/rsconnect/inspection-rf-api/"
...
print(ping)
reactable::reactable(enframe(metadata))
```

Return predictions from **your neighbor's** model API:

```{r}
url <- "https://quartz-shrimp.d01bf.fleeting.rstd.io/rsconnect/vetiver_rf_jrose/predict"
endpoint <- vetiver_endpoint(url)
predict(endpoint, slice_sample(inspect_test, n = 10))
```
...
```{r}
url <- "https://quartz-shrimp.d01bf.fleeting.rstd.io/rsconnect/vetiver_rf_jrose/"
req <- httr2::request(url) %>%
  httr2::req_url_path_append("prototype")
prototype <- req |>
  httr2::req_perform() |>
  httr2::resp_body_json()
